...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      if (isEnglishGameSpaPath(window.location.pathname)) {
        ...
        const legalView = legalViewFromPath(window.location.pathname);
        if (legalView && legalView !== globalView) {
  ...
  const navigateGlobalView = useCallback(
    (view: GlobalView, opts?: { authMode?: 'login' | 'register'; replace?: boolean }) => {
      setGlobalView(view);
      if (typeof window === 'undefined' || view === 'game' || view === 'admin') return;
      const nextUrl =
        view === 'auth' && opts?.authMode === 'register'
          ? `${legalPathFromView(view)}?auth=register`
          : legalPathFromView(view);
  ...
  const gameNavItems = useMemo<GameNavItem[]>(() => {
    ...
    const items: GameNavItem[] = [
      ...
      { key: 'wallet', label: gameNav('wallet'), icon: Wallet, accent: 'orange', section: 'economia', allowed: has('wallet') },
      {
        ...
        label: gameNav('withdrawal_history'),
        icon: History,
        accent: 'orange',
        section: 'economia',
        allowed: has('withdrawal_history')
      },
      ...
      { key: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, accent: 'sky', section: 'hub', allowed: !!user },
      { key: 'partners', label: gameNav('partners'), icon: Clapperboard, accent: 'violet', section: 'hub', allowed: has('partners') },
      {
        key: 'partner_games',
        label: gameNav('partner_games'),
        icon: Grid3X3,
        accent: 'violet',
        section: 'hub',
        allowed: has('partner_games')
      },
          ...
          {mobileMenuOpen && (
            <div className="w-full md:hidden">
              <div className="w-full grid grid-cols-1 gap-2">
                <button onClick={() => { navigateGlobalView('home'); setMobileMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${globalView === 'home' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Home size={16} /> Início</button>
                ...
                <button onClick={() => { setMobileMenuOpen(false); if (!user) navigateGlobalView('auth'); else { setGlobalView('game'); setCurrentView('ranking'); } }} className="flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border border-slate-200 dark:border-slate-700 text-yellow-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition"><Trophy size={16} /> Ranking</button>
                <button onClick={() => { navigateGlobalView('docs'); setMobileMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${globalView === 'docs' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><BookOpen size={16} /> Docs</button>
                {user && (globalView === 'home' || globalView === 'docs') && !user.isAdmin && (
                  <>
                ...
                {!user ? (
                  <button onClick={() => { navigateGlobalView('auth'); setMobileMenuOpen(false); }} className="flex items-center gap-2 px-3 py-2 text-sm font-bold rounded bg-gradient-to-r from-amber-400 to-amber-600 hover:from-amber-300 hover:to-amber-500 text-stone-950 border border-amber-300/40 shadow-md"><UserIcon size={16} /> Login</button>
                ) : (
                  <>
        ...
        {globalView === 'game' && user && (
          <>
            ...
            <nav className="min-w-0 w-full shrink-0 border-b border-slate-200 dark:border-amber-900/30 bg-gradient-to-b from-white to-slate-100 dark:from-slate-900/95 dark:to-[#0a0805] backdrop-blur-md shadow-[0_8px_28px_-14px_rgba(0,0,0,0.35)] dark:shadow-[0_12px_36px_-18px_rgba(245,158,11,0.07)] transition-colors duration-300 md:hidden">
              ...
              {gameNavExpanded && (
                <div className="max-w-7xl mx-auto px-4 pb-3 pt-2 space-y-3 border-t border-slate-800 bg-[#0a0805]/95">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {items.map((item) => {
                            ...
                            return (
                              <button
                                key={item.key}
                                onClick={() => { goToGameView(item.key); collapseGameNav(); }}
