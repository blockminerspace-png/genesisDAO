...
export function registerPartnerYoutubeRoutes(app: Express, deps: PartnerYoutubeDeps): void {
  ...
  async function loadPartnerNftRoomUserIds(userIds: number[]): Promise<Set<number>> {
  ...
  app.get('/api/admin/partner-youtube-partners', isAdmin, async (_req: Request, res: Response) => {
    try {
      ...
      const partnerUserIds = partnerRows
        .map((row) => Number(row.user_id))
        .filter((v) => Number.isFinite(v) && v > 0);
      const nftRoomActiveUserIds = await loadPartnerNftRoomUserIds(partnerUserIds);
      res.json({
