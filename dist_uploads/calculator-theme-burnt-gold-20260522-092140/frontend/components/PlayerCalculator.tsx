...
export const PlayerCalculator: React.FC<PlayerCalculatorProps> = ({ onBack }) => {
    ...
    const shellPanel = 'border border-amber-900/35 bg-[#120c06]';
    const mutedPanel = 'border border-amber-900/25 bg-[#171008]';

    return (
        <div className="flex-1 overflow-hidden bg-[#0f0c08] text-slate-200 flex">
            <div className="w-72 bg-[#120d08] border-r border-amber-900/25 flex flex-col p-5 shrink-0">
                <button
                    type="button"
                    onClick={onBack}
                    className="flex items-center gap-2 text-slate-400 hover:text-amber-300 mb-6 p-3 rounded-2xl hover:bg-[#1b1309] transition-colors"
                >
                    <ArrowLeft size={18} />
                ...
                <div className={`rounded-3xl p-4 mb-6 ${shellPanel}`}>
                    <div className="flex items-center gap-3 mb-3">
                        <div className="h-11 w-11 rounded-2xl border border-amber-800/40 bg-[#2a190b] text-amber-300 grid place-items-center">
                            <Pickaxe size={18} />
                        </div>
                        <div>
                            <div className="text-xs font-bold uppercase tracking-[0.24em] text-amber-300/80">Calculadora</div>
                    ...
                    {scopesUi.map((opt) => (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-3.5 rounded-2xl text-sm font-medium transition-all border ${
                                scope === opt.id
                                    ? 'bg-[#2a190b] text-amber-300 border-amber-700/50'
                                    : 'text-slate-400 border-transparent hover:bg-[#1a1209] hover:text-slate-200'
                ...
                <div className="mt-6 grid gap-3">
                    <div className={`rounded-2xl p-4 ${mutedPanel}`}>
                        <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-amber-300/80 mb-2">
                            <Layers3 size={14} />
                            ...
                            <div className={`rounded-[28px] p-5 md:p-6 ${shellPanel}`}>
                                <div className="flex flex-col gap-5">
                                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                                        ...
                                        {selectedCoin && (
                                            <div className="rounded-2xl border border-amber-900/35 bg-[#1d140b] px-4 py-3">
                                    ...
                                    {payload.coins.map((c) => (
                                        <button
                                            ...
                                            className={`px-4 py-2.5 rounded-2xl text-sm font-bold uppercase tracking-wider transition-all border sm:px-6 ${
                                                selectedCoinId === c.id
                                                    ? 'bg-[#8f5a16] text-white border-amber-400/50'
                                                    : 'text-slate-400 border-amber-900/25 hover:text-slate-200 hover:bg-[#1a1209]'
                                            }`}
                                        >
                                            {c.symbol || c.name}
                                        </button>
                                ...
                                <>
                                                ...
                                                >
                                                    <div className={`rounded-[22px] ${card.accent} p-5`}>
                                                        <div className="flex items-center justify-between mb-6">
                                                            ...
                                                            <div className="h-11 w-11 rounded-2xl border border-amber-900/30 bg-[#2a190b] grid place-items-center text-amber-300">
                                                                <Icon size={18} />
                                                            </div>
                                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 2xl:grid-cols-[1.25fr_0.95fr] gap-6">
                                        <div className={`rounded-[30px] p-6 md:p-8 ${shellPanel}`}>
                                            ...
                                            <div className="w-full overflow-hidden rounded-2xl border border-amber-900/20 bg-[#100b06]">
                                                <div className="grid grid-cols-3 pb-3 pt-4 border-b border-amber-900/20 text-[10px] items-center font-bold text-slate-500 uppercase tracking-[0.22em] px-4">
                                                    <div>Período</div>
                                                    ...
                                                    {selectedCoin.rows.map((period, index) => (
                                                        <div
                                                            ...
                                                            className={`grid grid-cols-3 py-4 px-4 items-center transition-colors ${
                                                                index !== selectedCoin.rows.length - 1 ? 'border-b border-amber-900/15' : ''
                                                            } hover:bg-[#191108]`}
                                                        >
                                        ...
                                        <div className={`rounded-[30px] p-6 md:p-8 ${shellPanel}`}>
                                            <div className="flex items-start justify-between gap-4 mb-6">
                                                ...
                                                <div className="rounded-2xl border border-amber-900/35 bg-[#1d140b] px-3 py-2 text-right">
                                                    <div className="text-[10px] uppercase tracking-[0.18em] text-amber-200/80">Total recente</div>
                                            ...
                                            {selectedBlockHistory.length === 0 ? (
                                                <div className="rounded-2xl border border-dashed border-amber-900/25 bg-[#120c06] px-4 py-10 text-center text-sm text-slate-500">
                                                ...
                                                <div className="space-y-3 max-h-[38rem] overflow-y-auto custom-scrollbar pr-1">
                                                    {selectedBlockHistory.map((item) => (
                                                        <div
                                                            key={item.id}
                                                            className="rounded-2xl border border-amber-900/20 bg-[#120c06] p-4 hover:bg-[#1a1209] transition-colors"
                                                        >
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3 mt-4 text-xs">
                                                                <div className="rounded-xl bg-[#1a1209] border border-amber-900/20 p-3">
