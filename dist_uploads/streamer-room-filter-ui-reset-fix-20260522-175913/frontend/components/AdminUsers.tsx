...
import { AdminSuspiciousEmailsPanel } from './AdminSuspiciousEmailsPanel';
...
export const AdminUsers: React.FC<AdminUsersProps> = ({
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                const data = await getUsers(1, 50, em, 'creation', 'asc', 'all', 'all', false, 'all');
                if (cancelled) return;
    ...
    const handleDeactivateStreamerRoom = async () => {
        ...
        if (!uid) return;
        const hasStreamerRoom = (selectedUserSave.placedRacks || []).some((rack) => String(rack.roomId || '').trim() === 'room_1766898636697');
        if (!hasStreamerRoom) {
        ...
        try {
            const res = await deactivateStreamerRoomByAdmin(uid, selectedUserSave);
            if (!res.ok) {
            ...
            if (refreshed.data) {
                setSelectedUserSave(refreshed.data);
            }
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col h-[500px]">
                    <div className="flex flex-col gap-2 mb-3 border-b border-slate-700 pb-2 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button
                            ...
                            disabled={saveTab === 'logs' || saveTab === 'wallets'}
                            title={
                            ...
                            className={`shrink-0 text-xs font-bold flex items-center gap-1 self-end sm:self-auto ${
                                saveTab === 'logs' || saveTab === 'wallets' ? 'text-slate-500 cursor-not-allowed' : 'text-green-400 hover:text-green-300'
                            }`}
                        >
