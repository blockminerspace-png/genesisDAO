...
export async function updateUser(user: User & { newReferralFor?: string }): Promise<{ ok: boolean; error?: string; code?: string; accounts?: any[]; requiresEmailVerification?: boolean; message?: string }> {
  try {
    ...
    return { ok: true, ...(typeof data === 'object' && data ? data : {}) } as { ok: boolean; error?: string; code?: string; accounts?: any[]; requiresEmailVerification?: boolean; message?: string };
  } catch {
    return { ok: false, error: 'Network error' };
  }
}
