...
export function mapUpgradeRowToApi(r, compatibleRacks = []) {
    return {
        id: r.id,
        name: r.name,
        category: r.category,
        ...
        nftMiningCoinId: r.nft_mining_coin_id != null && String(r.nft_mining_coin_id).trim()
            ? String(r.nft_mining_coin_id).trim()
            : undefined,
        asicDurationKind: r.asic_duration_kind != null && String(r.asic_duration_kind).trim()
            ? String(r.asic_duration_kind).trim()
            : 'none',
        asicDurationAmount: Math.max(0, Math.floor(Number(r.asic_duration_amount) || 0)),
        asicDurationUnit: r.asic_duration_unit != null && String(r.asic_duration_unit).trim()
...
export function resolveAsicDurationUpsertFields(payload, existing) {
    const hasExplicit = Object.prototype.hasOwnProperty.call(payload, 'asicDurationAmount') ||
        Object.prototype.hasOwnProperty.call(payload, 'asicDurationUnit');
    if (hasExplicit) {
        const amt = Math.floor(Number(payload.asicDurationAmount) || 0);
        const unit = typeof payload.asicDurationUnit === 'string' && payload.asicDurationUnit.trim()
    ...
    const unit = existing?.asic_duration_unit != null && String(existing.asic_duration_unit).trim()
