...
export default function App() {
  ...
  const getAllowedPages = (): string[] => {
    ...
    // Jogos do parceiro (/partner-games): derivado de Parceiros (não precisa checkbox extra no admin).
    if (pages.includes('partners') && !pages.includes('partner_games')) {
  ...
  useEffect(() => {
    ...
    const gated: Array<{ view: View; requiredPage: string }> = [
      ...
      { view: 'withdrawal_history', requiredPage: 'withdrawal_history' },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const isEmailVerificationUrl = Boolean(resetTokenParam && path.includes('verificar-email'));
        ...
        if (sess) {
          setUser(sess);
          if (isPasswordResetUrl || isEmailVerificationUrl) setGlobalView('auth');
  ...
  const handleWithdrawCoin = useCallback(async (coinId: string, amt: number): Promise<WalletWithdrawResult> => {
    ...
    if (!user?.polygonWallet || !coin) {
      return { ok: false, error: 'Conecte uma carteira de saque para levantar cripto.' };
    }
  ...
  return (
    <div className="h-screen min-h-0 w-full min-w-0 max-w-full flex flex-col bg-slate-50 dark:bg-[#0f0c08] text-slate-800 dark:text-slate-200 font-sans selection:bg-amber-500/30 overflow-hidden transition-colors duration-300">
      ...
      <main id="main-content" className="flex-1 min-h-0 min-w-0 overflow-hidden relative flex flex-col">
        {globalView === 'home' && <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col"><div className="flex-1 min-w-0"><HomePage onNavigate={setGlobalView} /></div><Footer onNavigate={setGlobalView} /></div>}
        {globalView === 'docs' && (
          <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col">
            ...
            <Footer onNavigate={setGlobalView} />
          </div>
        )}
        {globalView === 'terms' && (
          <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col">
          ...
          <>
            ...
            <nav className="min-w-0 w-full shrink-0 border-b border-slate-200 dark:border-amber-900/30 bg-gradient-to-b from-white to-slate-100 dark:from-slate-900/95 dark:to-[#0a0805] backdrop-blur-md shadow-[0_8px_28px_-14px_rgba(0,0,0,0.35)] dark:shadow-[0_12px_36px_-18px_rgba(245,158,11,0.07)] transition-colors duration-300 md:hidden">
              ...
              >
                ...
                <button
                  ...
                  aria-label={gameNavExpanded ? 'Ocultar menu' : 'Mostrar menu'}
                  className="p-2 rounded-lg text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                >
                  {gameNavExpanded ? <X size={18} /> : <Menu size={18} />}
                </button>
              </div>
              {gameNavExpanded && (
                <div className="max-w-7xl mx-auto px-4 pb-3 pt-2 grid grid-cols-1 gap-2 border-t border-amber-200/50 dark:border-amber-900/25 bg-slate-50/90 dark:bg-[#0a0805]/80">
                  {getAllowedPages().includes('servers') && (<button onClick={() => { goToGameView('servers'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'servers' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Server size={16} /> {gameNav('servers')}</button>)}
                  {getAllowedPages().includes('inventory') && (<button onClick={() => { goToGameView('inventory'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'inventory' ? 'border-yellow-500 text-yellow-600 dark:text-yellow-500' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Package size={16} /> {gameNav('inventory')}</button>)}
                  {getAllowedPages().includes('hardware_store') && (<button onClick={() => { goToGameView('hardware_store'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'hardware_store' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><ShoppingCart size={16} /> {gameNav('hardware_store')}</button>)}
                  {getAllowedPages().includes('black_market') && (<button onClick={() => { goToGameView('black_market'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'black_market' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Skull size={16} /> {gameNav('black_market')}</button>)}
                  {getAllowedPages().includes('arcade') && (<button onClick={() => { goToGameView('arcade'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'arcade' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Gamepad2 size={16} /> {gameNav('arcade')}</button>)}
                  {getAllowedPages().includes('lucky_store') && (<button onClick={() => { goToGameView('lucky_store'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'lucky_store' ? 'border-orange-500 text-orange-600 dark:text-orange-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Gift size={16} /> {gameNav('lucky_store')}</button>)}
                  {getAllowedPages().includes('roleta') && showRoletaInNav && (<button onClick={() => { goToGameView('roleta'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'roleta' ? 'border-rose-500 text-rose-600 dark:text-rose-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Sparkles size={16} /> {gameNav('roleta')}</button>)}
                  {getAllowedPages().includes('wallet') && (<button onClick={() => { goToGameView('wallet'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'wallet' ? 'border-orange-500 text-orange-600 dark:text-orange-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Wallet size={16} /> {gameNav('wallet')}</button>)}
                  {getAllowedPages().includes('withdrawal_history') && (<button onClick={() => { goToGameView('withdrawal_history'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'withdrawal_history' ? 'border-orange-500 text-orange-600 dark:text-orange-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><History size={16} /> {gameNav('withdrawal_history')}</button>)}
                  {getAllowedPages().includes('ranking') && (<button onClick={() => { goToGameView('ranking'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'ranking' ? 'border-yellow-500 text-yellow-600 dark:text-yellow-500' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Trophy size={16} /> {gameNav('ranking')}</button>)}
                  {getAllowedPages().includes('upgrade') && (<button onClick={() => { goToGameView('upgrade'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'upgrade' ? 'border-yellow-500 text-yellow-600 dark:text-yellow-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Crown size={16} /> {gameNav('upgrade')}</button>)}
                  {getAllowedPages().includes('transparency') && (<button onClick={() => { goToGameView('transparency'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'transparency' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Scale size={16} /> {gameNav('transparency')}</button>)}
                  {getAllowedPages().includes('support') && (<button onClick={() => { goToGameView('support'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'support' ? 'border-sky-500 text-sky-600 dark:text-sky-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><LifeBuoy size={16} /> {gameNav('support')}</button>)}
                  {getAllowedPages().includes('partners') && (<button onClick={() => { goToGameView('partners'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'partners' ? 'border-violet-500 text-violet-600 dark:text-violet-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Clapperboard size={16} /> {gameNav('partners')}</button>)}
                  {getAllowedPages().includes('partner_games') && (<button onClick={() => { goToGameView('partner_games'); collapseGameNav(); }} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded border ${currentView === 'partner_games' ? 'border-violet-500 text-violet-600 dark:text-violet-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}><Grid3X3 size={16} /> {gameNav('partner_games')}</button>)}
                </div>
              )}
            </nav>
            ...
            >
              <aside
                ...
                } sticky top-24 self-start mt-4 ml-4 mr-3 max-h-[calc(100vh-7rem)] flex-col overflow-hidden rounded-xl border border-slate-200/80 dark:border-amber-900/30 bg-white/85 dark:bg-[#0a0805]/88 backdrop-blur-md shadow-[0_18px_48px_-26px_rgba(15,23,42,0.55)] dark:shadow-[0_20px_60px_-28px_rgba(245,158,11,0.16)] transition-all duration-300`}
              >
                <div className="flex items-center justify-between gap-2 border-b border-slate-200/80 dark:border-amber-900/25 px-3 py-3">
                  {gameNavExpanded ? (
                    <div className="min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-700/90 dark:text-amber-500/90">Operação</p>
                    ...
                    <span className="mx-auto text-[10px] font-black uppercase tracking-[0.2em] text-amber-700/90 dark:text-amber-500/90">Nav</span>
                  )}
                  <button
                    type="button"
                    onClick={toggleGameNavExpanded}
                    ...
                    className={`shrink-0 rounded-lg border p-2 transition-all ${
                      gameNavExpanded
                        ? 'border-amber-400/55 text-amber-900 dark:text-amber-200 bg-amber-50/90 dark:bg-amber-950/45'
                        : 'border-amber-500/65 text-amber-800 dark:text-amber-100 bg-amber-500/20 dark:bg-amber-500/15 ring-1 ring-amber-400/25'
                    }`}
                  >
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar p-2">
                  <div className="flex flex-col gap-2">
                    {getAllowedPages().includes('servers') && (<button type="button" onClick={() => { goToGameView('servers'); }} className={`${gameNavTabClass(currentView === 'servers', 'amber')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('servers')}><Server size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('servers')}</span> : null}</button>)}
                    {getAllowedPages().includes('inventory') && (<button type="button" onClick={() => { goToGameView('inventory'); }} className={`${gameNavTabClass(currentView === 'inventory', 'yellow')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('inventory')}><Package size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('inventory')}</span> : null}</button>)}
                    {getAllowedPages().includes('hardware_store') && (<button type="button" onClick={() => { goToGameView('hardware_store'); }} className={`${gameNavTabClass(currentView === 'hardware_store', 'amber')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('hardware_store')}><ShoppingCart size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('hardware_store')}</span> : null}</button>)}
                    {getAllowedPages().includes('black_market') && (<button type="button" onClick={() => { goToGameView('black_market'); }} className={`${gameNavTabClass(currentView === 'black_market', 'red')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('black_market')}><Skull size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('black_market')}</span> : null}</button>)}
                    {getAllowedPages().includes('arcade') && (<button type="button" onClick={() => { goToGameView('arcade'); }} className={`${gameNavTabClass(currentView === 'arcade', 'amber')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('arcade')}><Gamepad2 size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('arcade')}</span> : null}</button>)}
                    {getAllowedPages().includes('lucky_store') && (<button type="button" onClick={() => { goToGameView('lucky_store'); }} className={`${gameNavTabClass(currentView === 'lucky_store', 'orange')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('lucky_store')}><Gift size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('lucky_store')}</span> : null}</button>)}
                    {getAllowedPages().includes('roleta') && showRoletaInNav && (<button type="button" onClick={() => { goToGameView('roleta'); }} className={`${gameNavTabClass(currentView === 'roleta', 'rose')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('roleta')}><Sparkles size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('roleta')}</span> : null}</button>)}
                    {getAllowedPages().includes('wallet') && (<button type="button" onClick={() => { goToGameView('wallet'); }} className={`${gameNavTabClass(currentView === 'wallet', 'orange')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('wallet')}><Wallet size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('wallet')}</span> : null}</button>)}
                    {getAllowedPages().includes('withdrawal_history') && (<button type="button" onClick={() => { goToGameView('withdrawal_history'); }} className={`${gameNavTabClass(currentView === 'withdrawal_history', 'orange')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('withdrawal_history')}><History size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('withdrawal_history')}</span> : null}</button>)}
                    {getAllowedPages().includes('ranking') && (<button type="button" onClick={() => { goToGameView('ranking'); }} className={`${gameNavTabClass(currentView === 'ranking', 'yellow')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('ranking')}><Trophy size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('ranking')}</span> : null}</button>)}
                    {getAllowedPages().includes('upgrade') && (<button type="button" onClick={() => { goToGameView('upgrade'); }} className={`${gameNavTabClass(currentView === 'upgrade', 'yellow')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('upgrade')}><Crown size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('upgrade')}</span> : null}</button>)}
                    {getAllowedPages().includes('transparency') && (<button type="button" onClick={() => { goToGameView('transparency'); }} className={`${gameNavTabClass(currentView === 'transparency', 'emerald')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('transparency')}><Scale size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('transparency')}</span> : null}</button>)}
                    {getAllowedPages().includes('support') && (<button type="button" onClick={() => { goToGameView('support'); }} className={`${gameNavTabClass(currentView === 'support', 'sky')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('support')}><LifeBuoy size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('support')}</span> : null}</button>)}
                    {getAllowedPages().includes('partners') && (<button type="button" onClick={() => { goToGameView('partners'); }} className={`${gameNavTabClass(currentView === 'partners', 'violet')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('partners')}><Clapperboard size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('partners')}</span> : null}</button>)}
                    {getAllowedPages().includes('partner_games') && (<button type="button" onClick={() => { goToGameView('partner_games'); }} className={`${gameNavTabClass(currentView === 'partner_games', 'violet')} w-full justify-start ${gameNavExpanded ? '' : 'px-0 justify-center'}`} title={gameNav('partner_games')}><Grid3X3 size={16} className="shrink-0 opacity-90" /> {gameNavExpanded ? <span className="truncate">{gameNav('partner_games')}</span> : null}</button>)}
                  </div>
                </div>
              </aside>
              ...
              >
                ...
                <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden custom-scrollbar relative min-h-0 flex flex-col font-mono">
                  ...
                  {saveLoaded && currentView === 'withdrawal_history' && getAllowedPages().includes('withdrawal_history') && (
                    <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
                      <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden custom-scrollbar">
                        <Suspense fallback={<LazyRouteFallback />}>
